...
import { opsApi, companiesApi, meApi } from "../services/api";
...
export default function AddOrderModal({ open, onClose, onCreated }) {
  ...
  const meQ = useQuery({
    ...
    staleTime: 60_000,
    enabled: open, // solo cuando el modal está abierto
  });
  ...
  // Prefijar la empresa del usuario no-staff al abrir
  useEffect(() => {
    if (!open) return;
    if (!isStaff && myEmpresaId) {
      setForm((f) => ({ ...f, empresa: String(myEmpresaId) }));
    }
  }, [open, isStaff, myEmpresaId]);
  ...
  const createMut = useMutation({
    ...
    onSuccess: () => {
      ...
      onCreated?.();
      setForm({
        empresa: isStaff ? "" : String(myEmpresaId || ""),
        excursion: "",
        fecha_inicio: "",
        fecha_fin: "",
        estado: "pendiente_pago",
            ...
            {isStaff ? (
              ...
              <label className="text-sm">
                ...
                <input
                  ...
                  value={meQ.data?.empresa_name || ""}
                  readOnly
                  disabled
                />
                {/* Enviamos el id real en form.empresa */}
